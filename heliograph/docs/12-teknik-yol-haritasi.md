...
- [~] **M0.12 Kiracı modeli** (domain: rol/izin matrisi, authorize, changeRole; Drizzle `workspaces/operators/memberships/invitations` + RLS + `withWorkspace` + çapraz kiracı testleri hazır; ADR-0014. `@CurrentWorkspace()` (409 `workspace.none_active`) ve `DomainErrorException` → RFC 9457 eşlemesi hazır. Kalan: `Authorizer` portu ile use-case düzeyinde izin kontrolü (ilk yazma uç noktası M1.1 ile))
