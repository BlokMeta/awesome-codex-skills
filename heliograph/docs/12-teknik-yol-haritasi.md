...
- [~] **M0.7 UI paketi çekirdeği** (token'lar hazır; `packages/ui` web: Button/IconButton/Badge/Skeleton/EmptyState/TextInput/Switch/Sheet/List (sanal + cursor "daha fazla") — paylaşımlı props arayüzleri `props/`, Testing Library + axe testleri, Storybook 10 (tema/yön araç çubuğu, a11y addon) ve CI'da `storybook:build`. Kalan: RN ikizleri (Unistyles) mobil iskeletle (M1.5), Storybook RN, görsel regresyon) — token'lar (Style Dictionary), yazı tipleri, `Button/Input/List/Sheet` web + RN; Storybook (web + RN); axe.
...
- [x] **M0.9 Gözlemlenebilirlik temeli** — pino + requestId + redact + traceId/spanId mixin; OTel NodeSDK (OTLP/HTTP, env ile açılır), `@fastify/otel` istek span'leri, undici, `db.transaction` span'leri, `hg_config_stale_keys` gauge; compose'daki otel-collector + Grafana'ya bağlı (pano tanımları M2'de).
...
- [x] **M0.13 Billing çekirdeği** — `plans`/`plan_entitlements` (tohum: Free/Solo/Creator/Studio/Agency/Internal), `subscriptions`, `credit_ledger` (balance_after, danışma kilidi), `usage_records.feature`; `EntitlementService.resolve/assert/overview` (abonelik yoksa Free), `GET /v1/billing/entitlements`; sağlayıcı entegrasyonu M1.6.
- [~] **M0.14 Privacy çekirdeği** — `consent_records` + `ConsentService` (yayımlı sürüm config'ten, kabul/geri çekme, IP+UA), `GET/POST /v1/consents`, `/v1/consents/withdraw`; `erasure_requests` tablosu hazır. Kalan (M2.8/M4.6): hesap silme (30 gün), veri dışa aktarma, Meta Data Deletion Callback + onay kodu sayfası, herkese açık silme sayfası.
- [~] **M0.12 Kiracı modeli** (domain: rol/izin matrisi, authorize, changeRole; Drizzle `workspaces/operators/memberships/invitations` + RLS + `withWorkspace` + çapraz kiracı testleri hazır; ADR-0014. `@CurrentWorkspace()` (409 `workspace.none_active`) ve `DomainErrorException` → RFC 9457 eşlemesi hazır. `Authorizer` (identity/application) ile use-case düzeyinde izin kontrolü PersonaService'te devrede — **tamam**)
...
- [~] **M1.1 Persona CRUD + sihirbaz (web)** — backend hazır: domain profilleri (ses kitabı, görsel kit, konu profili, etkileşim/kalite politikaları, `createDraft/readyToStart/archive`), `personas` + `persona_versions` (RLS, her değişiklik sürümlenir), `PersonaService` (authorize → entitlement → domain → persist), `/v1/personas` CRUD + activate/pause/archive (cursor sayfalama, 201/422/429 problem'ler), `Authorizer` portu (M0.12 kapanışı). Web: `/personas` (sanal liste, cursor "daha fazla"), `/personas/new`, `/personas/{id}/wizard` 6 adım (kimlik → ses kitabı + konular → görsel kit + canlı önizleme → kanallar (M1.2 yer tutucu) → politika (aralıklar, yayın pencereleri, sapma, etkileşim, insan onayı) → önizleme/başlat/duraklat/arşivle); her adım PATCH ile sürümlenir; `SelectField` ve `TagInput` UI paketine eklendi; bileşen testleri + Playwright e2e (axe temiz). Kalan: ses kitabı AI taslağı (`LlmWriter`, Anthropic adaptörü) — M2.2 ile birlikte.
  - KK: e2e "persona sihirbazı"; sahte dil ile taşma yok; RTL story.
- [~] **M1.2 Channel modülü** — backend hazır: domain `Channel` (sağlık, `healthFromExpiry` — refresh token'lı kısa ömürlü erişim token'ları uyarı değildir, `canPublish`), zarf şifreleme (`envelope.ts`: kimlik bilgisi başına AES-256-GCM DEK, `HG_ENCRYPTION_MASTER_KEY` ile sarılı; `KeyWrapper` arayüzü KMS için hazır), `credentials`/`channels`/`oauth_states` tabloları (RLS), config güdümlü OAuth 2.0 adaptörü (`platform.oauth` seed'inden uç noktalar ve scope'lar; Instagram/Threads kısa→uzun ömürlü token takası, TikTok/YouTube/X PKCE, X token uç noktasında Basic auth; `whoAmI` kimlik çağrıları), Telegram `getMe`, `ChannelService` (authorize → kota → sağlayıcı → domain → persist; çalışma alanları arası mükerrer hesap koruması; aynı hesap yeniden bağlanınca kimlik bilgisi yerinde döner; `refreshToken` zamanlayıcı giriş noktası), `/v1/channels/*` + public callback (302 ile web'e), 13 HTTP testi (sahte fetch ile X/Instagram/Telegram akışları, kota 429, RLS, düz metin sızıntısı yok). Kalan: web `/channels` sayfası + sihirbaz kanallar adımı; token yenileme cron'u (M2.6 zamanlayıcı ile); TikTok/YouTube gerçek uç nokta testleri sandbox anahtarları gelince.
