...
16. **Önce yeniden kullan, sonra yaz.** Bir problem için olgun bir açık kaynak kütüphane, referans uygulama, skill veya MCP sunucusu varsa **önce o değerlendirilir**; kendi yazımımız yalnızca (a) lisans uyumsuz, (b) bakımsız, (c) sınır ihlali gerektiriyor veya (d) ürün farkı yaratan çekirdek mantık ise. Liste ve karar notları `docs/13 §6`. Yeni bir bağımlılık eklemek = lisans + bakım durumu + yıldız/son commit kontrolü, PR açıklamasına yazılır.
